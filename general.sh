...
$radirecodir -s $station -l $location -t $duration -p '$program' && rclone sync "$HOME/RadikoSimulatorRecordings/$program/$program$next_date" "takuya:/$program" && echo "$next_dateに予約された$programの録音が完了しました" | mail -s "$programの予約録音完了" lv.kjsn.29yr@gmail.com
